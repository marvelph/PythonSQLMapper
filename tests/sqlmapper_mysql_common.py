...
class MySQLMapperTestMixin:
    DRIVER = None
    DRIVER_MISSING_REASON = "driver is not installed"
    ...
    def setUpClass(cls):
        if cls.DRIVER is None:
            raise unittest.SkipTest(cls.DRIVER_MISSING_REASON)
        ...
        if missing:
            raise unittest.SkipTest(
                "MySQL test env is not configured. Missing: {0}".format(", ".join(missing))
            )
        cls.connect_params = cls.build_connect_params(env)

    def setUp(self):
        self.mapper = Mapper(self.DRIVER, **self.connect_params)
        self._drop_tables()
        self._create_tables()
        self._seed_data()
    ...
    def _drop_tables(self):
        for table in ("audit_logs", "accounts", "users", "departments", "external_keys"):
            self.mapper.execute("DROP TABLE IF EXISTS {0}".format(table))
    ...
    def test_insert_without_auto_increment_returns_zero(self):
        self.mapper.execute(
            """
    ...
    def test_execute_can_run_special_sql(self):
        ...
        self.mapper.commit()
        columns = list(self.mapper.select_all("SHOW COLUMNS FROM users"))
        self.assertTrue(any(column.Field == "profile" for column in columns))
    ...
    def test_uncommitted_changes_are_rolled_back_when_exiting_with_by_exception(self):
        with self.assertRaises(RuntimeError):
            with Mapper(self.DRIVER, **self.connect_params) as mapper:
                mapper.update(
                    "UPDATE users SET status = :status WHERE id = :id",
                    {"id": self.bob_id, "status": "inactive"},
                )
        ...
        with Mapper(self.DRIVER, **self.connect_params) as checker:
